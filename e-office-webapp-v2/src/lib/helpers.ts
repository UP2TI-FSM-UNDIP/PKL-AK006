...
/**
 * Sanitizes a file URL to use the backend proxy if it points to an internal MinIO endpoint.
 * Matches: http://[host]:[port]/[bucket]/[path]
 * Converts to: /api/public/files/[path]
 *
 * @param url - The original file URL.
 * @returns The sanitized URL.
 */
export function sanitizeFileUrl(url: string | null | undefined): string {
  if (!url) return '';

  const basePath = process.env.NEXT_PUBLIC_BASE_PATH || '';

  // If it's already a relative URL (starts with /), prepend BASE_PATH if needed
  if (url.startsWith('/')) {
    // Don't double-prefix if BASE_PATH is already present
  ...
  try {
    ...
    // If it points to a known internal-only endpoint (localhost or 9000 port reachable only on server)
    if (urlObj.hostname === 'localhost' || urlObj.port === '9000') {
      const pathParts = urlObj.pathname.split('/').filter(Boolean);
      // part[0] is usually the bucket name (e.g., bucket-tim-2)
      const cleanPath = pathParts.slice(1).join('/');
      return `${basePath}/api/public/files/${cleanPath}`;
    }

    return url;
  } catch (err) {
    // If it's not a valid URL (e.g. malformed string), return it as-is
    return url;
  }
}
