...
// Fetch /me dengan Bearer token untuk verifikasi sesi dan ambil roles
...
// Role to redirect path mapping (sama dengan auth/page.tsx)
...
function SSOCallbackComponent() {
  ...
  useEffect(() => {
    ...
    const activate = async () => {
      try {
        // Step 1: Panggil set-session — backend akan HMAC-sign token dan set cookie BA
        ...
        // Step 2: Simpan token ke localStorage (untuk API calls yang pakai Bearer)
        setAuthToken(token);

        // Step 3: Ambil data user + roles
        ...
        const msg = err instanceof Error ? err.message : 'Terjadi kesalahan saat proses SSO.';
        setError(msg);
      }
    };
