...
import { letterApi, getStatusLabel, getStatusColor, formatDate, type LetterInstance } from "@/lib/api";
...
export default function DashboardPage() {
	...
	const [showStats, setShowStats] = useState(() => {
		if (typeof window !== 'undefined') {
			const saved = localStorage.getItem('upa-dashboard-show-stats');
		...
		const fetchData = async () => {
			try {
				...
				const [pendingResponse, processedResponse] = await Promise.all([
					letterApi.getUPAPendingLetters(),
					letterApi.getUPAProcessedLetters()
				]);

				console.log('[UPA Dashboard] Raw Pending Response:', pendingResponse);
				console.log('[UPA Dashboard] Raw Processed Response:', processedResponse);
				...
				if (pendingResponse.success || processedResponse.success) {
					...
					console.log('[UPA Dashboard] Pending Data:', pendingData);
					...
					console.log('[UPA Dashboard] Processed Data:', processedData);
					...
					const processedMapped: TableRow[] = processedData.map((item: LetterInstance) => ({
						...
						tglLabel: formatDate(item.createdAt),
						tglISO: item.createdAt.split('T')[0],
						tujuan: item.status === 'COMPLETED' ? 'Selesai (Arsip)' : 'Mahasiswa',
						status: getStatusLabel(item.status),
						statusColor: getStatusColor(item.status),
						needsAction: false,
					}));

					// Combine all letters for table
					const allLetters = [...pendingMapped, ...processedMapped];
					console.log('[UPA Dashboard] All Letters:', allLetters);
	...
	const handleCardClick = (filterType: string) => {
		...
		if (filterType === "pending") {
			// Perlu Tindakan: show letters that are pending at UPA (tujuan = "UPA")
			setStatusFilter("all");
			setTujuanFilter("UPA");
			setDateFrom("");
			setDateTo("");
	...
	return (
		<div className="min-h-screen flex flex-col">
			...
			<div className="flex flex-1 min-w-0 bg-[#F3F3F3] dark:bg-gray-900">
				...
				<main className="flex-1 min-w-0 overflow-x-hidden px-4 sm:px-6 lg:px-24 pt-6 sm:pt-8 pb-8">
					...
					<div className="flex items-center justify-between mb-6 sm:mb-8">
						...
						<button
							onClick={() => {
								...
								setShowStats(next);
								localStorage.setItem('upa-dashboard-show-stats', String(next));
							}}
									...
									{(() => {
										...
										return (
											<svg width="100%" height="100%" viewBox="0 0 560 140" preserveAspectRatio="xMidYMid meet">
												<defs>
													<linearGradient id="chartGradientUPA" x1="0" y1="0" x2="0" y2="1">
												...
												{areaD && <path d={areaD} fill="url(#chartGradientUPA)" />}
					...
					<div ref={tableRef} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 transition-colors">
						<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-5 gap-3">
							<span className="font-semibold text-base sm:text-lg dark:text-white">Semua Surat</span>
							<div className="flex flex-col sm:flex-row gap-2">
								<input
									className="w-full sm:w-auto border dark:border-gray-600 rounded px-3 py-1.5 sm:py-1 text-sm bg-white dark:bg-gray-700 dark:text-gray-200 transition-colors"
									placeholder="Cari surat..."
									value={query}
									onChange={(e) => setQuery(e.target.value)}
								/>
								<div className="hidden sm:flex items-center gap-2 border dark:border-gray-600 rounded px-3 py-1 text-sm bg-gray-50 dark:bg-gray-700 transition-colors">
								...
								<select
									className="w-full sm:w-auto border dark:border-gray-600 rounded px-3 py-1.5 sm:py-1 text-sm bg-gray-50 dark:bg-gray-700 dark:text-gray-200 transition-colors"
									value={statusFilter}
							...
							<table className="min-w-full text-sm">
								...
								<tbody>
									{isLoading ? (
										...
										pagedRows.map((row) => (
											<tr key={row.letterId} className={`border-b transition-colors ${row.needsAction ? 'border-amber-200 dark:border-amber-800 bg-amber-50/60 dark:bg-amber-900/15 hover:bg-amber-100/80 dark:hover:bg-amber-900/25' : 'dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
												<td className="py-2 px-2 font-mono dark:text-gray-300">{row.id}</td>
												<td className="py-2 px-2"><span className="bg-gray-100 dark:bg-gray-700 rounded px-2 py-1 text-xs dark:text-gray-200">{row.sumber}</span></td>
						...
						<div className="flex flex-col sm:flex-row justify-between items-center mt-2 text-xs text-gray-500 dark:text-gray-400 gap-4">
							...
							<div className="flex gap-1 items-center">
								<button
									className="border dark:border-gray-600 rounded px-2 py-1 dark:text-gray-300 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
									disabled={safePage <= 1}
								...
								{Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
									...
									return (
										<button
											key={pageNum}
											className={`border dark:border-gray-600 rounded px-2 py-1 transition-colors ${isActive ? "bg-blue-600 text-white" : "dark:text-gray-300 dark:hover:bg-gray-700"}`}
