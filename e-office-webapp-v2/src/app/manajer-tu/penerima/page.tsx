	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				setIsLoading(true);
				setError(null);
				const response = await letterApi.getMTUPendingLetters();
		...
		<div className="min-h-screen flex flex-col">
			...
			<div className="flex flex-1 min-w-0 bg-[#F3F3F3] dark:bg-gray-900">
				...
				<main className="flex-1 min-w-0 overflow-x-hidden px-4 sm:px-6 lg:px-12 xl:px-24 pt-6 sm:pt-8 pb-8">
					...
					<h1 className="text-2xl font-bold mb-1 dark:text-white">Penerima</h1>
					<p className="mb-6 text-gray-500 dark:text-gray-400">Penerima</p>

					{/* Filter Panel */}
					<div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm dark:shadow-gray-900/50 p-4 sm:p-6 mb-6 transition-colors">
						<h2 className="font-semibold mb-4 sm:mb-6 text-base sm:text-lg dark:text-white">Filter Pencarian</h2>
					...
					{/* Table - Desktop */}
					<div className="bg-white dark:bg-gray-800 rounded-xl shadow dark:shadow-gray-900/50 p-4 sm:p-6 transition-colors">
						...
						<div className="md:hidden space-y-3">
							{isLoading ? (
								...
								<div className="py-8 text-center text-gray-500 dark:text-gray-400">Tidak ada surat yang menunggu tanda tangan</div>
							) : pagedRows.map((row) => (
								<div key={row.letterId} className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 border border-gray-100 dark:border-gray-700">
									<div className="flex items-start justify-between mb-2">
										<p className="font-medium text-sm text-gray-900 dark:text-white">{row.pengirim}</p>
										<span className="flex items-center gap-1.5 text-xs">
											<span className="w-2 h-2 rounded-full" style={{ background: row.statusColor }} />
											<span className="dark:text-gray-300">{row.status}</span>
										</span>
									</div>
									<p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{row.perihal}</p>
									<p className="text-xs text-gray-400 dark:text-gray-500 mb-3">{row.tglLabel}</p>
									<button
										onClick={() => router.push(`/manajer-tu/penerima/identitas-pemohon?id=${row.letterId}`)}
										className="w-full text-center py-2 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg text-xs font-medium hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors"
									>
										Lihat Detail
									</button>
								</div>
