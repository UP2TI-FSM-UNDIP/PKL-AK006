...
function PratinjauSuratPageContent() {
	...
	if (isLoading || templateLoading) {
		return (
			<div className="min-h-screen flex flex-col">
				...
				<div className="flex flex-1 bg-[#F3F3F3] dark:bg-gray-900 items-center justify-center">
	...
	if (error) {
		return (
			<div className="min-h-screen flex flex-col">
				<TopBar role="Manajer TU" onMenuClick={() => setSidebarOpen(true)} />
				<div className="flex flex-1 bg-[#F3F3F3] dark:bg-gray-900 items-center justify-center px-4">
				...
				<main className="flex-1 min-w-0 overflow-x-hidden px-3 sm:px-6 lg:px-12 pt-4 sm:pt-8 pb-8">
					...
					<div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3">
						<div className="flex items-center gap-2">
							<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="dark:stroke-gray-300 shrink-0">
						...
						<div className="flex items-center justify-between sm:justify-end gap-3">
							<div className="flex items-center gap-2 sm:gap-3 bg-white dark:bg-gray-800 px-3 sm:px-4 py-2 rounded-lg shadow-sm dark:shadow-gray-900/50 transition-colors">
								<button onClick={handleZoomOut} className="text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white font-semibold text-lg transition-colors" title="Zoom Out">−</button>
								<button onClick={handleZoomReset} className="text-gray-700 dark:text-gray-200 font-medium min-w-[50px] sm:min-w-[60px] text-center hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded transition-colors text-sm">{zoom}%</button>
								<button onClick={handleZoomIn} className="text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white font-semibold text-lg transition-colors" title="Zoom In">+</button>
							</div>
							<div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">Halaman 1 dari 1</div>
						</div>
					</div>

					{/* Document Viewer */}
					<div ref={containerRef} className="bg-[#E5E5E5] dark:bg-gray-700 rounded-lg p-2 sm:p-4 lg:p-8 overflow-auto transition-colors min-h-[400px] sm:min-h-[600px]">
						<div style={{ width: `${visualWidth}px`, height: `${visualHeight}px`, margin: '0 auto', position: 'relative' }}>
							<div className="bg-white shadow-2xl" style={{ width: `${A4_WIDTH}px`, position: 'absolute', top: 0, left: 0, transform: `scale(${combinedScale})`, transformOrigin: 'top left' }}>
								<AK006LetterTemplate
									data={templateData}
					...
					<div className="mt-4 sm:mt-6">
						<button
							onClick={() => router.back()}
							className="w-full sm:w-auto px-6 py-2.5 sm:py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 font-medium text-sm transition-colors"
						>
							Kembali
						</button>
					</div>
