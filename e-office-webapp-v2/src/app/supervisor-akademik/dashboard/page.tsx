...
type TableRow = {
    ...
    status: string;
    statusColor: string;
    needsAction: boolean; // true if this letter needs action from the current user
};
...
export default function DashboardPage() {
    ...
    const [showStats, setShowStats] = useState(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem('sa-dashboard-show-stats');
        ...
        const fetchData = async () => {
            try {
                ...
                const [pendingResponse, processedResponse] = await Promise.all([
                    letterApi.getSAPendingLetters(),
                    letterApi.getSAProcessedLetters()
                ]);

                console.log('[SA Dashboard] Raw Pending Response:', pendingResponse);
                console.log('[SA Dashboard] Raw Processed Response:', processedResponse);
                ...
                if (pendingResponse.success || processedResponse.success) {
                    ...
                    console.log('[SA Dashboard] Pending Data:', pendingData);
                    ...
                    const pendingMapped: TableRow[] = pendingData.map((item: LetterInstance) => {
                        return {
                            ...
                            tujuan: "Supervisor Akademik", // Pending di SA
                            status: getStatusLabel(item.status),
                            statusColor: getStatusColor(item.status),
                            needsAction: true, // Pending letters need SA action
                        };
                    });

                    // Map processed letters
                    const processedData = processedResponse.success && processedResponse.data ? processedResponse.data : [];
                    console.log('[SA Dashboard] Processed Data:', processedData);
                    ...
                    const processedMapped: TableRow[] = processedData.map((item: LetterInstance) => {
                        ...
                        // Logic khusus untuk status display
                        if (item.status === 'COMPLETED') {
                            ...
                            statusColor = '#FB923C';
                        } else if (item.currentStep === 1 && item.status === 'REVISION') {
                            // Status Revisi aktif di level SA (menunggu Mahasiswa/Target)
                            // Meskipun currentStep 1, jika status REVISION, berarti menunggu perbaikan
                            tujuanLabel = 'Mahasiswa';
                            statusLabel = 'Perlu Revisi';
                            statusColor = '#FB923C';
                        } else {
                            // Normal flow or Step > 1
                            // Do not override based on history
                        }
                        ...
                        return {
                            ...
                            needsAction: tujuanLabel === 'Supervisor Akademik' && statusLabel === 'Menunggu Verifikasi',
                        };
                    });

                    // Combine all letters for table with deduplication
                    // Create a map to store unique letters by ID, prioritizing pending letters
                    const uniqueLetters = new Map<string, TableRow>();

                    // Add pending letters first (priority - shows "Action Needed" state)
                    pendingMapped.forEach((item) => {
                        uniqueLetters.set(item.letterId, item);
                    });

                    // Add processed letters only if not already present
                    // This prevents duplicates when a letter is both "processed" (historically) and "pending" (resubmitted)
                    processedMapped.forEach((item) => {
                        if (!uniqueLetters.has(item.letterId)) {
                            uniqueLetters.set(item.letterId, item);
                        }
                    });
                    ...
                    console.log('[SA Dashboard] All Letters (Deduplicated):', allLetters);
    ...
    const handleCardClick = (filterType: string) => {
        ...
        if (filterType === "pending") {
            // Perlu Tindakan: show all pending regardless of month
            setStatusFilter("Menunggu Verifikasi");
            setDateFrom("");
            setDateTo("");
        } else {
            // Selesai & Total: filter by current month (January 2026)
            const now = new Date();
        ...
        <div className="min-h-screen flex flex-col bg-white dark:bg-gray-900 transition-colors duration-300">
            ...
            <div className="flex flex-1 min-w-0 bg-[#F3F3F3] dark:bg-gray-900 transition-colors duration-300">
                ...
                <main className="flex-1 min-w-0 overflow-x-hidden px-4 sm:px-6 lg:px-24 pt-6 sm:pt-8 pb-8">
                    ...
                    <div className="flex items-center justify-between mb-6 sm:mb-8">
                        ...
                        <button
                            onClick={() => {
                                ...
                                setShowStats(next);
                                localStorage.setItem('sa-dashboard-show-stats', String(next));
                            }}
                        ...
                        >
                            <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="transition-transform duration-200">
                                {showStats ? (
                    ...
                    {showStats && (<>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                            <CardDashboard
                                ...
                                isActive={statusFilter === "Menunggu Verifikasi"}
                                color="orange"
                                highlight
                            />
                            <CardDashboard
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                            {/* Chart */}
                            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 lg:col-span-2 flex flex-col transition-colors duration-300">
                                    ...
                                    {(() => {
                                        ...
                                        return (
                                            <svg width="100%" height="100%" viewBox="0 0 560 140" preserveAspectRatio="xMidYMid meet">
                                                <defs>
                                                    <linearGradient id="chartGradientSA" x1="0" y1="0" x2="0" y2="1">
                                                ...
                                                {areaD && <path d={areaD} fill="url(#chartGradientSA)" />}
                            ...
                            {/* Status Distribution */}
                            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 flex flex-col transition-colors duration-300 min-h-[100px]">
                    ...
                    {/* Table */}
                    <div ref={tableRef} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 sm:p-6 transition-colors duration-300">
